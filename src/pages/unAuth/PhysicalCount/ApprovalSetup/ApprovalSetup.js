...
import {
  ...
  generateNewEntry,
  searchInputMock,
} from "../PhysicalCountUtils";
import { CustomTable } from "./CustomTable";
import { Loader } from "../../../../components/Loader";
import {
  createApproverPhysicalCount,
  deleteApproverPhysicalCount,
  getApproversPhysicalCountOrgId,
  getOrganizationCode,
  physicalCountSearchList,
  physicalCountSearchListWithoutPagination,
  updateApproverPhysicalCount,
} from "../../../../services/api";
...
export const ApprovalSetup = () => {
  ...
  const initialCall = async () => {
    try {
      setLoading(true);
      const api = await getOrganizationCode();
  ...
  const onClickSearch = async () => {
    setLoading(true);
    try {
      const approverAPIRes = await getApproversPhysicalCountOrgId(
        searchInput.value
      );
      if (!approverAPIRes.data.data)
        throw new Error("Error in getting Approvers");

      const getOrgIdAPI = await physicalCountSearchListWithoutPagination({
      ...
      const tabdata = generateApprovalSetupTableData(
        data,
  ...
  const onClickDeleteRow = async (i) => {
    ...
    try {
      if (selectedRow.fromDB) {
        const api = await deleteApproverPhysicalCount(selectedRow.id);
  ...
  const onClickSave = async () => {
    ...
    const dataToCreate = tableData
      .filter((el) => !el.fromDB)
      .map((el) => createPayloadCreate(el, organizationId, userDetails));

    try {
      await Promise.all(
        dataToCreate.map(async (paramsData) => {
          return await createApproverPhysicalCount(paramsData);
        })
      );

      await Promise.all(
        dataToUpdate.map(async (paramsData) => {
          return await updateApproverPhysicalCount(paramsData);
        })
      );
  ...
  const onClickSingleRow = async (row, type, index) => {
    try {
      if (type === "create") {
        const createApi = await createApproverPhysicalCount(
          createPayloadCreate(row, organizationId, userDetails)
        );
        ...
        const updateApi = await updateApproverPhysicalCount(
