...
export const EmployeeFilterDetail = (props) => {
    ...
    useEffect(() => {
        console.log('s=====', searchObj.name.length != 0, searchObj.id.length != 0, (searchObj.name.length != 0 || searchObj.id.length != 0))
        if ((searchObj.name.length != 0 || searchObj.id.length != 0)) {

            var localArr = (searchObj.name.length == 0 && searchObj.id.length == 0) ? commonlogic()
    ...
    const checkAllSelect = () => {
        ...
        console.log('localArr.length == filterData.length', localArr.length == filterData.length)
    ...
    const onChangeSelectAll = (value) => {
        ...
        var localArray = oriPagedata?.map((item, index) => ({ ...item, checked: value }))
        setFilterData(localArray)

    }

    return <CustomDialog maxWidth="sm" dialogTitle='Filter By Employee' open='true' handleClose={handleClose}>
                    ...
                    <Grid container alignItems='center'>
                        <Grid item xs='4'>
                            <Typography fontSize='14px'>
                                <Box textAlign='right' mr={2}>Employee Number</Box>
                            </Typography>
                        </Grid>
                        <Grid item xs='8' >
                            <Box>
